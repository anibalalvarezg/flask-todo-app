...
def index(): 
    db, c = get_db()
    c.execute(
        'select t.id, t.description, u.username, t.completed, t.created_at from todo t join user u on u.id = t.created_by order by created_at desc'
    )
    todos = c.fetchall()
...
def get_todo(id):
    ...
    c.execute(
        ...
        'from todo t join user u on u.id=t.created_by where t.id=%s',
        (id,)
    )
